...
#define OBJ_FMT "\t%s.sem();\n"
//#define OBJ_FMT "\t%s = (%s*)mem_alloc(sizeof(%s));\n"

void agg_object(const char *tipo_object, const char *nome_object)
{
	char *buf;
	int dim;

	dim = strlen(OBJ_FMT) + strlen(nome_object);

	buf = xmalloc(dim);

#if defined WIN || defined WIN_XP
	sprintf(buf, OBJ_FMT, nome_object);
	agg_riga(MAIN, buf);
#else
	sprintf(buf, dim, OBJ_FMT, nome_object);
	agg_riga(MAIN, buf);
#endif
...
void object()
{
	char tipo_object_figlio[LUN_NOME];
	char nome_object_figlio[LUN_NOME];

	salta_spazi();
	leggi_nome(1);
	strcpy(tipo_object_figlio, nome);

	
	leggi_nome(1);
	strcpy(nome_object_figlio, nome);
	trova(';', 1);
	emetti(tipo_object_figlio);
	emetti(" ");
	emetti(nome_object_figlio);
	emetti(";\n");

	agg_object(tipo_object_figlio, nome_object_figlio);
}
